...
A personal assistant is one channel, one conversation — it answers everything I ask in a single ongoing thread; in this project that channel is Telegram by default. Under the hood, it's an *evolving knowledge base, wired into the network, that happens to talk back*.

The bet: **own the one layer that doesn't churn — your data.** The model keeps evolving, the harness is throwaway glue, even the CLI underneath could swap out — but the folder stays mine.
...
Most personal-AI products want to be the system of record: your life pours into their database, hostage to someone else's uptime and business model. Persona is the other way round — the one thing that makes it mine is **my data**, plain Markdown in a folder I control: readable, greppable, deletable. The assistant is disposable; the data is permanent.

Everything *above* the data is swappable — the model is a commodity I can trade down, the harness is throwaway glue, and even the skills are distilled from my own logs, so the judgment survives any format. *Freedom isn't running the strongest model — it's being able to swap it.*
...
**You grow your own skills.** A skill is just a Markdown prompt. The assistant improves not from a bigger model but from skills shaped to me — distilled from my own conversation logs, or generated by whatever meta skill-builder I point at them, until each one fits my life instead of a vendor's defaults. A correction I make once becomes a rule; a need I keep hitting becomes a skill that's already there next time.
